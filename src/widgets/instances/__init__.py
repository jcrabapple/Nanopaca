...
from .. import dialog
from .openai_instances import NanoGPT, BaseInstance
...
class InstancePreferencesDialog(Adw.Dialog):
    ...
    def __init__(self, instance):
        ...
        self.set_title(
            _("Edit Instance") if self.instance.instance_id else _("Create Instance")
        )
        self.model_list = []

        # CONNECTION GROUP
        self.connection_group.set_title(self.instance.instance_type_display)
        self.connection_group.set_description(
            self.instance.description or self.instance.properties.get("url")
        )

        self.set_simple_element_value(self.name_el)

        self.set_simple_element_value(self.url_el)
        if self.instance.instance_type in (
            "ollama",
            "ollama:managed",
            "openai:generic",
        ):
            if self.instance.instance_type == "ollama:managed":
                try:
                    ...
                    port = 11435
                self.port_el.connect(
                    "notify::value",
                    lambda el, gparam: self.url_el.set_text(
                        "http://127.0.0.1:{}".format(int(el.get_value()))
                    ),
                )
        ...
        if "api" in self.instance.properties:
            normal_api_title = _(
                "API Key (Optional)"
                if self.instance.instance_type == "ollama"
                else _("API Key")
            )
            unchanged_api_title = _("API Key (Unchanged)")
            self.api_el.set_title(
                unchanged_api_title
                if self.instance.properties.get("api")
                else normal_api_title
            )
            self.api_el.connect(
                "changed",
                lambda el: el.set_title(
                    normal_api_title if el.get_text() else unchanged_api_title
                ),
            )
        ...
        if "keep_alive" in self.instance.properties:
            self.keep_alive_selector_el.set_selected(
                0
                if self.instance.properties.get("keep_alive", -1) > 0
                else self.instance.properties.get("keep_alive") + 2
            )
            self.keep_alive_preset_changed(self.keep_alive_selector_el)
            self.keep_alive_selector_el.get_ancestor(Adw.PreferencesGroup).set_visible(
                True
            )
        ...
        factory.connect(
            ...
            lambda factory, list_item: list_item.set_child(
                Gtk.Label(ellipsize=3, xalign=0)
            ),
        )
        factory.connect(
            "bind",
            lambda factory, list_item: list_item.get_child().set_label(
    ...
    def set_simple_element_value(self, el):
        if el.get_name().startswith("override:"):
            in_properties = el.get_name().removeprefix(
                "override:"
            ) in self.instance.properties.get("overrides", {})
        else:
        ...
        if in_properties:
            if el.get_name().startswith("override:"):
                value = self.instance.properties.get("overrides", {}).get(
                    el.get_name().removeprefix("override:")
                )
            else:
            ...
            elif isinstance(el, Adw.EntryRow) and not isinstance(
                el, Adw.PasswordEntryRow
            ):
                el.set_text(value)
    ...
    def save_requested(self, button=None):
        def save_elements_values(elements: list):
            for el in elements:
                if el.get_visible():
                    ...
                    elif key.removeprefix("override:") in self.instance.properties.get(
                        "overrides", {}
                    ):
                        self.instance.properties["overrides"][
                            key.removeprefix("override:")
                        ] = new_value
        ...
        if len(list(self.get_root().instance_listbox)) > 0:
            ...
            self.get_root().instance_manager_stack.set_visible_child_name(
                "no-instances"
            )
        self.instance.stop()
    ...
    def keep_alive_preset_changed(self, combo, gparam=None):
        ...
        if index == 0:
            self.keep_alive_minutes_el.set_adjustment(
                Gtk.Adjustment(
    ...
    def model_directory_requested(self, button):
        dialog.simple_directory(
            ...
            callback=lambda res, row=self.model_directory_el: row.set_subtitle(
                res.get_path() if res else row.get_subtitle()
            ),
        )
...
class Empty:
    ...
    properties = {"name": _("Fallback Instance")}
    ...
    def __init__(self, instance, pinned: bool = False):
        ...
        if not isinstance(self.instance, Empty):
            edit_button = Gtk.Button(
                icon_name="edit-symbolic", valign=3, css_classes=["accent", "flat"]
            )
...
def create_instance_row(ins: dict) -> InstanceRow or None:
    # Only support NanoGPT instances
    if ins.get("type") == "nanogpt":
        return InstanceRow(
            instance=NanoGPT(
                instance_id=ins.get("id"), properties=ins.get("properties")
            )
        )
    return None
...
def update_instance_list(instance_listbox: Gtk.ListBox, selected_instance_id: str):
    ...
    if len(instances) > 0:
        instance_listbox.get_root().instance_manager_stack.set_visible_child_name(
            "content"
        )
